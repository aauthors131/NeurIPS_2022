...
if __name__ == '__main__':
    ...
    data = CODEXDataModule(src_ch = len(channel_ids['source_channel_ids']), tgt_ch = len(channel_ids['target_channel_ids']), \
    ...
    data.prepare_data()
    test_dataloader = data.test_dataloader(test_mode = True)
    ...
    with torch.no_grad():
        pix2pix(test_dataloader)
    # trainer.eval(model = pix2pix, dataloaders=test_dataloader)
